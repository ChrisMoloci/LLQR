...
import unicodeToShiftJIS from "../src/datasets/unicode_to_shiftjis.js";
...
for (let i = 1; i <= 7094; i += 1) {
    ...
    allnumericData[testChars.length] = {
        data: testChars
    }
}

const numericFileName = "./../tests_sample_data/encoder_data_sets/numeric_sample_data.ts";
const numericJsonString = `const encoderNumericSampleData: Record <string, Record<string, string>> = ${JSON.stringify(allnumericData, null, 2) + "\n"}
export default encoderNumericSampleData;
`;
...
for (let i = 1; i <= 4300; i++) {
    ...
    allAlphanumericData[testChars.length] = {
        data: testChars
    }
}

const alphanumericFileName = "./../tests_sample_data/encoder_data_sets/alphanumeric_sample_data.json";
...
// Create Kanji data set
const unicodeKanjiValues = Object.keys(unicodeToShiftJIS).map((key) => String.fromCharCode(parseInt(key)));

console.log(`Total Kanji characters available: ${unicodeKanjiValues.length}`);

const allKanjiData = {};

for (let i = 1; i <= unicodeKanjiValues.length; i += 1) {
    const kanjiTestChars = unicodeKanjiValues.slice(0, i).join('');

    allKanjiData[kanjiTestChars.length] = {
        data: kanjiTestChars
    }
}

const kanjiFileName = "./../tests_sample_data/encoder_data_sets/kanji_sample_data.json";
const kanjiJsonString = JSON.stringify(allKanjiData, null, 2) + "\n";
writeFileSync(kanjiFileName, kanjiJsonString, "utf8");
console.log(`Wrote ${kanjiFileName}`);
...
for (let i = 1; i <= 2956; i += 1) {
    let testStringArray = [];
    for (let j = 0; j < i; j++) {
        testStringArray.push(latin1Chars[j % latin1Chars.length]);
    }
    let testString = testStringArray.join("");
    allLatin1Data[testString.length] = {
        data: testString
    }
}

const latin1FileName = "./../tests_sample_data/encoder_data_sets/ISO_8859_1_sample_data.json";
...
const ws = createWriteStream("./../tests_sample_data/encoder_data_sets/UTF8_sample_data.ndjson");
...
for (const [start, end] of ranges) {
  ...
  for (let cp = start; cp <= end; cp++) {
    ...
    if (buffer.length >= 10000) await flush();
  }
}
await flush();
ws.end();
console.log(`Wrote UTF8_sample_data.ndjson`);